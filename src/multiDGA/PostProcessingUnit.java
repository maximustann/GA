...
//    private ArrayList<ArrayList<MultiGroupGAChromosome>> dominantSetList;
    private ArrayList<ArrayList<DualPermutationChromosome>> dominantSetList;
    private ArrayList<double[]> genTime;
    private int run;
    private int population;
    private int generation;

    public PostProcessingUnit(ArrayList<ArrayList<DualPermutationChromosome>> dominantSetList,
                              ArrayList<double[]> genTime,
                              int generation, int population, int run){
...
//    }
    ...
    public double[][] allResults(){
        ...
        // For each generation, we have a front
        for(int i = 0; i < dominantSetList.size(); i++){
            ArrayList<DualPermutationChromosome> dominateFront = dominantSetList.get(i);
            int frontSize = dominateFront.size();
            //For each front
            for(int j = 0; j < frontSize; j++){
                paretoFront[counter][0] = dominateFront.get(j).getFitness();
                paretoFront[counter][1] = dominateFront.get(j).updateAverageAvailability();
                paretoFront[counter][2] = i;
                counter++;
            }
        }
    ...
    public double[] energy(){
        ...
        ArrayList<DualPermutationChromosome> lastSet = dominantSetList.get(generation - 1);

        for(int i = 0; i < lastSet.size(); i++){
            energyConsumption[0] += lastSet.get(i).getFitness();
...
//        double averageWaste = 0;
        double averageWaste = 0;
        ArrayList<DualPermutationChromosome> lastFront = dominantSetList.get(generation - 1);
    ...
    public double[] averageUtil(){
        ...
        ArrayList<DualPermutationChromosome> lastFront = dominantSetList.get(generation - 1);
        for(int i = 0; i < lastFront.size(); i++){
            DualPermutationChromosome solution = lastFront.get(i);
...
//        ArrayList<MultiGroupGAChromosome> lastGen = dominantSetList.get(generation - 1);
        ...
        ArrayList<DualPermutationChromosome> lastGen = dominantSetList.get(generation - 1);
        int averageVm = 0;
        for(int i = 0; i < lastGen.size(); i++){
            averageVm += lastGen.get(i).getNoOfUsedVm();
        }
