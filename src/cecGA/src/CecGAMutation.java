...
//        mutateVmAllocation((GroupGAGene) individual);
//
...
//        System.out.println("vmList.size() = " + vmList.size());
        ...
        ArrayList<Integer> leftOverContainer = new ArrayList<>();
        int index = 0;
        for(Map.Entry entry:vmList.entrySet()){
            ArrayList<Integer> vm = (ArrayList<Integer>) entry.getValue();
            int vmType = chromo.individual[vm.get(0) * 2];
            double vmCpuRemain = vmCpu[vmType] - vmCpuOverheadRate * vmCpu[vmType];
            double vmMemRemain = vmMem[vmType] - vmMemOverhead;
            for(int i = 0; i < vm.size(); ++i){
                if(vmCpuRemain < taskCpu[vm.get(i)] ||
                    vmMemRemain < taskMem[vm.get(i)]){
                    // find another VM with the same type to allocate this container
                    leftOverContainer.add(vm.get(i));
                } else{
                    // This VM has enough resources
                    vmCpuRemain -= taskCpu[vm.get(i)];
                    vmMemRemain -= taskMem[vm.get(i)];
                    chromo.individual[vm.get(i) * 2 + 1] = index;
                }

            }
            vmStatusList.add(new double[]{vmCpuRemain, vmMemRemain});
            index += 1;
        }

        // Finally, we reallocate the left over containers with FF
        for(Integer container:leftOverContainer){
            int vmCounter = 0;
            boolean flag = false;
            for(double[] vmStatus:vmStatusList){
                if(vmStatus[0] >= taskCpu[container] &&
                    vmStatus[1] >= taskMem[container]){
                    chromo.individual[container * 2 + 1] = vmCounter;
                    vmStatus[0] -= taskCpu[container];
                    vmStatus[1] -= taskMem[container];
                    flag = true;
                    break;
                }
            ...
            // cannot find a VM to allocate, create a new one
            if(!flag){
                int newVMType = chromo.individual[container * 2];
                double[] newVM = new double[]{vmCpu[newVMType] - vmCpuOverheadRate * vmCpu[newVMType] - taskCpu[container],
                                                vmMem[newVMType] - vmMemOverhead - taskMem[container]};
                chromo.individual[container * 2 + 1] = vmStatusList.size();
                vmStatusList.add(newVM);
            }
        }
...
//        ArrayList<Integer> vm3Containers = new ArrayList<>();
//
//        for(int i = 0; i < numOfContainer; ++i){
...
//    private void fix(GroupGAGene chromo, int chosenVm){
//        for(int i = 0; i < numOfContainer; ++i){
