...
public class EnergyFitness extends UnNormalizedFit {
	...
	private double energy(ArrayList<double[]> vmUsage){
		...
		for(int i = 0; i < vmUsage.size(); i++){
			...
			double[] pmStatus;
			double[] pmResBound;
			wastedCPU += vmCpu[vmType] - vm[0];
			wastedMEM += vmMem[vmType] - vm[1];
			ArrayList<int[]> pm;
		...
		for(int i = 0; i < pmStatusList.size(); i++){
			...
			totalEnergy += energy;
			wastedCPU += pmCpu - pmStatusList.get(i)[0];
			wastedMEM += pmMem - pmStatusList.get(i)[1];
		}

		double normalizedWastedCPU = wastedCPU / pmStatusList.size() / pmCpu;
		double normalizedWastedMEM = wastedMEM / pmStatusList.size() /pmMem;
		double wastedResources = normalizedWastedCPU < normalizedWastedMEM ? normalizedWastedCPU:normalizedWastedMEM;

		((CecGAChromosome) individual).setWastedResources(wastedResources);
